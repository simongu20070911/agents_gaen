...
class ChiefResearcherAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        create_dir_tool = toolset.get_tool("create_directory")
        yield await create_dir_tool.run_async(ctx, path=planning_dir)

        # Create the research plan using an LlmAgent
        llm_agent = LlmAgent(
        ...
        async for event in llm_agent.run_async(ctx):
            if event.content and event.content.parts:
                research_plan_content = event.content.parts[0].text
                write_file_tool = toolset.get_tool("write_file")
                yield await write_file_tool.run_async(
                    ctx, path=plan_path, content=research_plan_content
                )
                break
