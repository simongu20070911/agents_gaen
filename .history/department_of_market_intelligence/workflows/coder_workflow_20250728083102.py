...
class CoderWorkflowAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        try:
            # MCP tools require a valid tool_context. Pass the current InvocationContext.
            # The result may be a nested coroutine, so we await it twice.
            manifest_content_response = await await desktop_commander_toolset.get_tools()[0].run_async(
                args={'path': manifest_path}, tool_context=ctx
            )
